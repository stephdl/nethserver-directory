...
jQuery(document).ready(function () {
    ...
    $('#" . $view->getUniqueId('confirmNewPassword') . "').PasswordStrength({
        position: {my: 'right top', at: 'right bottom'},
        leds: [{ label: '" . $T('Password confirmation does not match') . "', test: function(value) { return value === $('#" . $view->getUniqueId('newPassword') . "').val() }}],
...
echo $view->header('username')->setAttribute('template', $T('Change password for user `${0}`'));
